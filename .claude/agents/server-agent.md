...
- `app/storage.py` — `GameStore` interface + implementation (swap-able for a DB).
...
2. Keep routes thin; business logic in `stats.py`, persistence behind
   `storage.py`. Replacing the in-memory store with a real DB must not require
   touching the routers.
3. Every new/changed endpoint gets a test. Run `pytest` before finishing.
4. Setup/run: `pip install -e ".[dev]"`, `uvicorn app.main:app --reload`.
